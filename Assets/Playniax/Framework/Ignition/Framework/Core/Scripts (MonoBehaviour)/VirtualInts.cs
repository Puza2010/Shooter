...
namespace Playniax.Ignition
{
    ...
    public class VirtualInts : MonoBehaviour
    {
        [System.Serializable]
        ...
        public static VirtualInts instance
        {
            get
            {
                return _Init();
        ...
        static VirtualInts _Init()
        {
            if (_instance == null)
            {
                VirtualInts[] instances = FindObjectsOfType<VirtualInts>();

                if (instances.Length == 0)
                {
                    _instance = new GameObject("Virtual Ints").AddComponent<VirtualInts>();
                }
        ...
        void _Merge(VirtualInts source)
        {
